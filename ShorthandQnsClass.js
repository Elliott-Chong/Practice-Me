...
class ShorthandTemp {
   ...
   */
    ...
    */
    generateShorthandQns(difficulty) {
        ...
        if (difficulty == '1' || this.generateRnd(10) % 3 == 0) {
            query += (rnd < 6) ? `${alpha} += ${this.generateRnd(8)}` : `${alpha} -= ${this.generateRnd(6)}`;
        } else if (difficulty == '2') {
            query += (rnd < 4) ? `${alpha} += ${this.generateRnd(8)}` :
                (rnd < 8) ? `${alpha} *= ${this.generateRnd(5)}` :
                    `${alpha} -= ${this.generateRnd(6)}`;
        }

        // performing arithmetic operation (+, -, *, %)
        if (difficulty == '1' && this.generateRnd(10) % 3 != 0) {
            query += (this.generateRnd(10) < 6) ? ` + ${this.generateRnd(6)}` : ` - ${this.generateRnd(3)}`;

        } else if (difficulty == '2' && this.generateRnd(10) % 5 != 0) {
            query += (rnd < 3) ? ` * ${this.generateRnd(3)}` :
                (rnd < 6) ? ` + ${this.generateRnd(6)}` :
                    (rnd <= 9) ? ` % ${this.generateRnd(3)}` :
                        ` - ${this.generateRnd(3)}`;
        }
